...
Prepared on September 3, 2026. Ravue 2.1.8 is based on the maintainer-accepted 2.1.7 implementation. Its only functional addition is direct image paste on the existing stable image-input page. The accepted file picker, drag/drop, direct-image, selector, preparation, and result flows remain in place.
...
| --- | --- | --- | --- |
| P01 | Open panel in English | English controls, version 2.1.8, existing selector button intact | Pending real Firefox |
| P02 | Open panel in Brazilian Portuguese | Portuguese controls selected from browser locale | Pending real Firefox |
| C01 | Copy a PNG/JPEG image, open Add an image, and press Ctrl+V (Command+V on macOS) | The same tab immediately prepares that image and continues to Lens | Pending real Firefox |
| C02 | Copy text or a URL and paste on the image-input page | Nothing is submitted; the page remains usable | Pending real Firefox |
...
| U01 | Install 2.1.8 over signed 2.1.7 | Same add-on ID; one upgraded extension, no duplicate | Pending signed AMO build |
